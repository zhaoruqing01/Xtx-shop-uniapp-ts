...
// 支付接口,开发环境无法正常支付,这里只写思路
const onGoPayment = async () => {
  // 如果是开发环境则调用开发接口
  if (import.meta.env.DEV) {
    const res = await getPayOrderAPI({ orderId: query.id })
    console.log(res)
  } else {
    // 生产环境：获取支付参数 + 发起微信支付
...
// 获取物流信息
const logisticsData = ref<LogisticItem[]>()
const getMemberOrderLogisticsData = async () => {
  const res = await getMemberOrderLogisticsByIdAPI(query.id)
  logisticsData.value = res.result.list
}
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        ...
        // 将收货后返回的数据重新赋值给orderDetail,以刷新页面
        orderDetail.value = res.result
...
const onDeleteOrder = () => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        const res = await delMemberOrderAPI({ ids: [query.id] })
        console.log(res)
    ...
    <template v-if="orderDetail?.id">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="orderDetail?.orderState === OrderState.DaiFuKuan">
          ...
          <view class="button" @tap="onGoPayment">去支付</view>
        </template>
        <!-- 其他订单状态:展示再次购买按钮 -->
        <template v-else>
          <!-- 订单状态文字 -->
          ...
          <view class="button-group">
            ...
            <view
              v-if="isEnv && orderDetail.orderState === OrderState.DaiFaHuo"
              @tap="onOrderSend"
              class="button"
            >
              模拟发货
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in logisticsData" :key="item.id" class="item">
          <view class="message">
            {{ item.text }}
          </view>
      ...
      <view class="goods">
        <view class="item">
          ...
          <!-- 待评价状态:展示按钮 -->
          <view class="action" v-if="orderDetail.orderState !== 1">
        ...
        <view class="total">
          <view class="row">
            ...
            <view class="symbol">{{ orderDetail?.totalMoney }}.00</view>
          </view>
          <view class="row">
            <view class="text">运费: </view>
            <view class="symbol">{{ orderDetail?.postFee }}.00</view>
          </view>
          <view class="row">
            <view class="text">应付金额: </view>
            <view class="symbol primary">{{ orderDetail?.payMoney }}.00</view>
          </view>
        </view>
      </view>
