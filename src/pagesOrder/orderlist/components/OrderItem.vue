...
import { getMemberOrderAPI, getPayOrderAPI, getPayWxPayMiniPayAPI } from '@/services/order'
...
const getOrderListData = async () => {
  ...
  orderListData.value = res.result.items
}
// 支付功能
const onGoPayment = async (id: string) => {
  // 如果是开发环境则调用开发接口
  if (import.meta.env.DEV) {
    const res = await getPayOrderAPI({ orderId: id })
  } else {
    // 生产环境：获取支付参数 + 发起微信支付
    const res = await getPayWxPayMiniPayAPI({ orderId: id })
    await wx.requestPayment(res.result)
    uni.showToast({ icon: 'success', title: '支付成功' })
    orderListData.value!.find((v) => v.id === id)!.orderState = OrderState.DaiShouHuo
  }
}
// 组件挂载到页面时加载
onMounted(() => {
  ...
  <scroll-view scroll-y class="orders">
    <view class="card" v-for="item in orderListData" :key="item.id">
      ...
      <view class="status">
        ...
        <text class="icon-delete" v-if="item.orderState > OrderState.DaiShouHuo"></text>
      </view>
      ...
      <view class="action">
        ...
        <template v-if="item.orderState === OrderState.DaiFuKuan">
          <view class="button primary" @tap="onGoPayment(item.id)">去支付</view>
        </template>
        <template v-else>
          <navigator
