...
const addInfo = computed(() => {
  return addressStore().addressInfo || orderPre.value?.userAddresses.find((v) => v.isDefault)
})

// 提交订单功能
const onSubmitOrder = async () => {
  if (!addInfo.value?.id) return uni.showToast({ icon: 'error', title: '请填写收货地址' })
  const res = await postOrderAPI({
    addressId: addInfo.value?.id,
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <navigator
      ...
      class="shipment"
      hover-class="none"
      url="/pagesMemeber/address/address?from=order"
    >
      <view class="user"> {{ addInfo.receiver }} {{ addInfo.contact }} </view>
      <view class="address"> {{ addInfo.fullLocation }} </view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: orderPre?.goods.length === 0 }" @tap="onSubmitOrder">
      提交订单
    </view>
  </view>
</template>
